...
class PRF_Final_CourseworkZ{
	...
	public static void AllOrders(){
		...
		for(int i=0;i<SizeArr.length;i++){
			...
			int[] StatusTemp=new int[OrderId.length];
		...
		System.out.println();
			...
			for(int i=0;i<OrderId.length;i++){
				...
				if(ID.equals(OrderId[i])){
							...
							System.out.println("\tStatus       : "+(status[i]==0?"processing":status[i]==1?"delivering":status[i]==2?"deliverded":""));
	...
	public static void PlaceOrder(){
		...
		L1:do{
			...
			status=extendArrayInteger(status);
