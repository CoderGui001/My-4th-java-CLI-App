...
class PRF_Final_Coursework{
	...
	public static int[] StatusReading=new int[0];
						...
						SortedAmount=Convert_String_Array_to_Double_array_And_SortIT(Amount);
							...
							double total=0;
								...
								for(int i=0;i<SortedAmount.length;i++){
											...
											if(PhoneNumber[i].equals(PN)){
												
												double[] QTYtemp=new double[QTYarr.length];
												if(QTYarr[i]*XS==SortedAmount[i]){
													QTYtemp[i]=QTYarr[i];	
												}
												
												System.out.printf("\t\t|%-10s|%-10s|%15s|%n","  ","  "+QTYtemp[i],"     "+df.format(SortedAmount[i])+"  ");
	...
	public static void PlaceOrder(){
		...
		L1:do{
			...
			StatusReading= extendArrayInteger(StatusReading);
			StatusReading[StatusReading.length-1]=Status;
