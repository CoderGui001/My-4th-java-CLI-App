		...
		for(int i=0;i<CustomerID.length;i++){
			...
			for(int j=0;j<PhoneNumber.length;j++){
				if(CustomerID[i].equals(PhoneNumber[j])){
					switch(SizeArr[j]){
						case "XS":QTYtemp[0]+=QTYarr[j];break;
						case "S":QTYtemp[1]+=QTYarr[j];break;
						case "M":QTYtemp[2]+=QTYarr[j];break;
						case "L":QTYtemp[3]+=QTYarr[j];break;
						case "XL":QTYtemp[4]+=QTYarr[j];break;
						case "XXL":QTYtemp[5]+=QTYarr[j];break;
				...
				System.out.printf("\t\t|%17s|%9s|%17s|%n","   "+CustomerID[i]+"   "," "+QTYtemp[i]+" ","   "+df.format(total)+"  ");
